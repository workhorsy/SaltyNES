...
Mapper018::Mapper018() : MapperDefault() {

}

shared_ptr<MapperDefault> Mapper018::Init(shared_ptr<NES> nes) {
	irq_counter = 0;
	irq_latch = 0;
	irq_enabled = false;
	//regs[] = new int[11];
...
void Mapper018::mapperInternalStateLoad(ByteBuffer* buf) {
...
void Mapper018::mapperInternalStateSave(ByteBuffer* buf) {
...
void Mapper018::write(int address, short value) {
...
void Mapper018::loadROM(ROM* rom) {
...
int Mapper018::syncH(int scanline) {
	if (irq_enabled) {
		if (irq_counter <= 113) {
